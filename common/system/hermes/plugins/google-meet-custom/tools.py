    ...
    "parameters": {
        ...
        "properties": {
            ...
            "language": {
                ...
                "enum": ["es-MX", "en-US"],
                "default": "es-MX",
                "description": "Meet caption source language. Defaults to Spanish (Mexico).",
            },
            "translated_captions": {
                "type": "boolean",
                "default": False,
                "description": "Enable translated captions. Off by default.",
            },
            "translation_target": {
                "type": ["string", "null"],
                "enum": ["es-MX", "en-US", None],
                "description": "Translation target; requires translated_captions=true.",
            },
        },
        "required": ["url"],
...
MEET_SET_LANGUAGE_SCHEMA: Dict[str, Any] = {
    "name": "meet_set_language",
    "description": "Change active Meet caption source language and verify it in settings.",
    "parameters": {
        "type": "object",
        "properties": {
            "language": {"type": "string", "enum": ["es-MX", "en-US"]},
            "translated_captions": {"type": "boolean", "default": False},
            "translation_target": {"type": ["string", "null"], "enum": ["es-MX", "en-US", None]},
            "node": {"type": "string"},
        },
        "required": ["language"],
        "additionalProperties": False,
...
def handle_meet_join(args: Dict[str, Any], **_kw) -> str:
    ...
    language = args.get("language") or "es-MX"
    if language not in {"es-MX", "en-US"}:
        return _err("language must be 'es-MX' or 'en-US'")
    translated = bool(args.get("translated_captions", False))
    target = args.get("translation_target")
    if target is not None and not translated:
        return _err("translation_target requires translated_captions=true")
...
def handle_meet_set_language(args: Dict[str, Any], **_kw) -> str:
    ...
    if client is not None:
        try:
            res = client.set_language(language, translated, target)
            return _json({"success": bool(res.get("ok")), "node": node_name, **res})
        except Exception as e:
            return _err(f"remote node set_language failed: {e}", node=node_name)
    res = pm.set_language(language=language, translated_captions=translated, translation_target=target)
    return _json({"success": bool(res.get("ok")), **res})
