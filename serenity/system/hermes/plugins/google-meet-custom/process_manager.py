...
def start(
    ...
    *,
    ...
    env["HERMES_MEET_LANGUAGE"] = language
    env["HERMES_MEET_TRANSLATED_CAPTIONS"] = "1" if translated_captions else "0"
    if translation_target:
        env["HERMES_MEET_TRANSLATION_TARGET"] = translation_target
    env["HERMES_MEET_PROFILE_DIR"] = persistent_profile_dir or str(_root() / "profile")
    default_auth = _root() / "auth.json"
    if not auth_state and default_auth.is_file():
...
def set_language(
    *, language: str, translated_captions: bool = False,
    translation_target: Optional[str] = None,
) -> Dict[str, Any]:
    """Queue a live settings command for the bot process."""
    active = _read_active()
    if not active:
        return {"ok": False, "reason": "no active meeting"}
    out_dir = Path(active.get("out_dir", ""))
    ...
    with (out_dir / "commands.jsonl").open("a", encoding="utf-8") as f:
        f.write(json.dumps(command) + "\n")
    results_path = out_dir / "command_results.jsonl"
    deadline = time.monotonic() + 15
    while time.monotonic() < deadline:
        if results_path.is_file():
            for line in reversed(results_path.read_text(encoding="utf-8", errors="replace").splitlines()):
                try:
                    result = json.loads(line)
                except json.JSONDecodeError:
                    continue
                if result.get("id") == command_id:
                    return {"meetingId": active.get("meeting_id"), **result}
        time.sleep(0.25)
    return {
        "ok": False,
        "meetingId": active.get("meeting_id"),
        "error": "timed out waiting for Meet to verify caption language",
    }
